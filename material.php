				...
				<div class="col-sm-9 padding-right">
					<div class="features_items"><!--features_items-->
						...
						<?php
							...
							if($cari){
								if($inputcari != ""){
									$sql = mysqli_query($koneksi, "SELECT * FROM material WHERE no_sap like '%$inputcari%' OR nama_barang like '%$inputcari%' OR satuan like '%$inputcari%' OR stock like '%$inputcari%' OR lokasi like '%$inputcari%' ORDER BY no_sap DESC");
									
								}else{
    								$sql = mysqli_query($koneksi, "SELECT * FROM material ORDER BY no_sap DESC LIMIT $posisi, $batas");
								}
							}else{
								$sql = mysqli_query($koneksi, "SELECT * FROM material ORDER BY no_sap DESC LIMIT $posisi, $batas");
							}
							if(mysqli_num_rows($sql) == 0){
								...
								//hitung jumlah data
						$jml_data = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM material"));
						//Jumlah halaman
						...
						if ( $i == $pg ) {
						$nmr .= "<li class='active'><a href='?pg=$i'>$i</a></li>";
						} else {
